  ...
  return (
    <div className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-lg border border-gray-100 transition-all duration-300 flex flex-col">
      ...
      <Link href={`/listings/${listing.id}`} className="flex flex-col flex-1">
        {/* Photo */}
        <div className={`relative h-44 lg:h-52 bg-gradient-to-br ${style.gradient} flex items-center justify-center overflow-hidden`}>
          {coverPhoto ? (
            ...
            <>
              ...
              />
              <span className="absolute bottom-2.5 left-3 bg-black/50 backdrop-blur-sm text-white text-[10px] font-semibold px-2 py-0.5 rounded-md">
          ...
          <span className="absolute top-3 left-3 bg-black/30 backdrop-blur-sm text-white text-[10px] lg:text-[11px] font-semibold px-2 py-0.5 lg:py-1 rounded-md">
        ...
        <div className="p-3 lg:p-4 flex flex-col flex-1">
          ...
          <div className="flex items-end justify-between mb-1.5">
            <div>
              ...
              <p className="text-lg lg:text-xl font-black text-[#1e3a5f] leading-none">{formatPrice(listing.price, listing.listingType)}</p>
            </div>
            {psf !== null && (
              <div className="text-right">
                <p className="text-[10px] text-gray-400"><T en="Per sq.ft." bm="Setiap kaki²" /></p>
                <p className="text-sm font-semibold text-gray-400">RM {psf.toLocaleString('en-MY')}</p>
              </div>
            )}
          </div>

          {/* PSF bar — filled proportionally as a visual indicator */}
          <div className="h-1.5 rounded-full bg-gray-100 overflow-hidden mb-2.5">
            <div className="h-full rounded-full bg-[#1e3a5f]" style={{ width: psf ? `${Math.min(100, Math.round((psf / 1500) * 100))}%` : '50%' }} />
          </div>

          {/* Title + location */}
          <h3 className="font-semibold text-gray-900 text-sm leading-snug line-clamp-2 mb-1 group-hover:text-[#1e3a5f] transition-colors">
            {listing.title}
          </h3>
          {location && (
            <p className="text-gray-500 text-xs flex items-center gap-1 mb-2.5">
            ...
            <div className="grid grid-cols-3 gap-1.5 mb-2.5">
              {beds && (
                <div className="bg-slate-50 rounded-lg p-1.5 lg:p-2 text-center">
                  <p className="text-[9px] lg:text-[10px] text-gray-400"><T en="Beds" bm="Bilik" /></p>
                  <p className="text-[11px] lg:text-xs font-bold text-gray-800">{beds}</p>
                </div>
              )}
              {baths && (
                <div className="bg-slate-50 rounded-lg p-1.5 lg:p-2 text-center">
                  <p className="text-[9px] lg:text-[10px] text-gray-400"><T en="Baths" bm="Mandi" /></p>
                  <p className="text-[11px] lg:text-xs font-bold text-gray-800">{baths}</p>
                </div>
              )}
              {size && (
                <div className="bg-slate-50 rounded-lg p-1.5 lg:p-2 text-center">
                  <p className="text-[9px] lg:text-[10px] text-gray-400"><T en="Size" bm="Keluasan" /></p>
                  <p className="text-[11px] lg:text-xs font-bold text-gray-800 truncate">{size}</p>
                </div>
              )}
            </div>
          )}
      ...
      {/* Action row */}
      <div className="px-3 lg:px-4 pb-3 lg:pb-4 flex items-center gap-2">
        {listing.featured ? (
          <Link
            href="/contact"
          ...
          <a
            ...
            className="flex-1 flex items-center justify-center gap-1.5 bg-green-500 hover:bg-green-600 text-white text-xs font-semibold py-2.5 lg:py-3 rounded-xl transition-colors"
          >
