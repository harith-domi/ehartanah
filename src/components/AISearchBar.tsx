      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-2xl p-4 sm:p-5">
        ...
        <div className="flex flex-wrap gap-2 mt-3 mb-4">
          {quickChips.map((chip) => (
            <button
              ...
              className="text-xs bg-[#edf2f8] text-[#1e3a5f] hover:bg-[#dce8f0] px-3 py-1.5 rounded-full transition-colors font-medium border border-[#dce8f0]"
            >
              {chip}
            </button>
          ))}
        ...
        <div className="flex items-center justify-between gap-3 border-t border-gray-100 pt-3">
          ...
          <button
            type="submit"
            disabled={!query.trim()}
            className="flex items-center gap-2 bg-gradient-to-r from-[#0f2540] via-[#1e3a5f] to-[#1e3a5f] hover:opacity-90 text-white font-semibold px-6 py-2.5 rounded-xl transition-opacity disabled:opacity-40 disabled:cursor-not-allowed ml-auto"
          >
