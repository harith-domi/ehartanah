      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-2xl p-4 sm:p-5">
        ...
        <div className="flex flex-wrap gap-2 mt-3 mb-4">
          {quickChips.map((chip) => (
            <button
              ...
              className="text-xs bg-blue-50 text-blue-700 hover:bg-blue-100 px-3 py-1.5 rounded-full transition-colors font-medium border border-blue-100"
            >
              {chip}
            </button>
          ))}
        ...
        <div className="flex items-center justify-between gap-3 border-t border-gray-100 pt-3">
          ...
          <button
            type="submit"
            disabled={!query.trim()}
            className="flex items-center gap-2 bg-gradient-to-r from-blue-600 via-violet-600 to-purple-700 hover:opacity-90 text-white font-semibold px-6 py-2.5 rounded-xl transition-opacity disabled:opacity-40 disabled:cursor-not-allowed ml-auto"
          >
