...
function MessageBubble({ message }: { message: Message }) {
  ...
  return (
    <div className={`flex gap-3 ${isUser ? 'flex-row-reverse' : 'flex-row'} mb-4`}>
      {/* Avatar */}
      {!isUser && (
        <div className="w-9 h-9 rounded-full bg-gradient-to-br from-blue-600 to-violet-700 flex items-center justify-center shrink-0 mt-1">
...
export default function ChatInterface({ initialQuery }: { initialQuery?: string }) {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 rounded-2xl overflow-hidden border border-gray-200">
      {/* Header */}
      <div className="bg-gradient-to-r from-blue-700 to-violet-700 px-4 py-3 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-blue-200 text-xs">Powered by property intelligence</p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 min-h-0">
        {isEmpty ? (
          <div className="flex flex-col items-center justify-center h-full text-center py-8">
            <div className="w-16 h-16 rounded-full bg-gradient-to-br from-blue-100 to-violet-100 flex items-center justify-center mb-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 w-full max-w-lg">
              {EXAMPLE_PROMPTS.slice(0, 6).map((prompt) => (
                <button
                  ...
                  className="text-left text-xs bg-white border border-gray-200 hover:border-blue-300 hover:bg-blue-50 text-gray-700 px-3 py-2.5 rounded-xl transition-colors"
                >
                  {prompt}
                </button>
              ))}
          ...
          <>
            ...
            {thinking && (
              <div className="flex gap-3 mb-4">
                <div className="w-9 h-9 rounded-full bg-gradient-to-br from-blue-600 to-violet-700 flex items-center justify-center shrink-0">
                  <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
                  </svg>
                </div>
      ...
      <div className="border-t border-gray-200 bg-white p-3">
        <div className="flex gap-2 items-end">
          <textarea
            ...
            className="flex-1 resize-none border border-gray-200 rounded-xl px-3 py-2.5 text-sm outline-none focus:border-blue-400 transition-colors leading-relaxed"
            rows={1}
          ...
          <button
            ...
            className="w-10 h-10 bg-gradient-to-r from-blue-600 to-violet-700 rounded-xl flex items-center justify-center disabled:opacity-40 disabled:cursor-not-allowed hover:opacity-90 transition-opacity shrink-0"
            aria-label="Send"
          >
            <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
