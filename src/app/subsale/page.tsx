...
import { saleListings, uniqueRegions } from '@/lib/listings';
import FilterBar from '@/components/FilterBar';
import FilteredListings from '@/components/FilteredListings';
import T from '@/components/T';

export const metadata: Metadata = {
  title: 'Subsale Properties in Malaysia | eHartanah',
  description:
    'Browse subsale houses, condos, land and commercial property across Malaysia. Filter by area, price, type and bedrooms. Direct from private owners and agencies.',
};
...
export default function SubsalePage() {
  ...
  const regions = uniqueRegions(saleListings);
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      ...
      <div className="bg-[#0f2540] text-white">
        <div className="max-w-7xl mx-auto px-4 py-10 lg:py-14">
          <h1 className="text-2xl lg:text-4xl font-black leading-tight mb-2">
            <T en="Subsale Properties in Malaysia" bm="Hartanah Subsale di Malaysia" />
          </h1>
          <p className="text-blue-200 text-sm lg:text-base mb-6">
            <T
              en={`${saleListings.length.toLocaleString('en-MY')} properties for sale — houses, condos, land & commercial`}
              bm={`${saleListings.length.toLocaleString('en-MY')} hartanah untuk dijual — rumah, kondominium, tanah & komersial`}
            />
          </p>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {top8.map(({ region, count }) => (
                <Link
                  key={region}
                  href={`/properties/subsale/${toSlug(region)}`}
      ...
      <Suspense fallback={null}>
        <FilterBar regions={regions} listingType="sale" />
      </Suspense>

      {/* Filtered listings */}
      <FilteredListings listings={saleListings} />
    </main>
  );
}
