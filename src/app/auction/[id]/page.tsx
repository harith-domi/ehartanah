  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="bg-white border-b border-gray-100 py-3">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 text-sm text-gray-500 flex gap-2">
          <Link href="/" className="hover:text-[#1e3a5f]">Home</Link>
          <span>/</span>
          <Link href="/auction" className="hover:text-[#1e3a5f]">Auction</Link>
          <span>/</span>
          <span className="text-gray-800 truncate">{l.propertyType}</span>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
        {/* Back */}
        <div className="mb-5">
          <Link href="/auction" className="inline-flex items-center gap-1.5 text-[#1e3a5f] hover:text-[#0f2540] hover:bg-[#edf2f8] font-semibold text-sm px-3 py-1.5 -ml-3 rounded-xl transition-colors">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-5">
            {/* Photo */}
            <div className="relative rounded-2xl overflow-hidden h-72 bg-gray-800">
              {photo ? (
              ...
              {l.auctionDate && (
                <div className="absolute top-0 left-0 flex items-center">
                  <div className="w-2 h-14 bg-gray-900" />
                  <div className="bg-orange-500 px-4 h-14 flex items-center">
                    <span className="text-white font-black text-2xl tracking-tight">{l.auctionDate}</span>
                  </div>
                </div>
              )}
              {/* EST stamp */}
              <div className="absolute top-3 right-4 w-14 h-14 flex items-center justify-center">
                <div className="absolute inset-0 bg-amber-100 border-2 border-amber-300"
                  style={{ clipPath: 'polygon(50% 0%,61% 35%,98% 35%,68% 57%,79% 91%,50% 70%,21% 91%,32% 57%,2% 35%,39% 35%)' }}
                />
                <span className="relative text-[11px] font-black text-amber-800">EST.</span>
              </div>
              {/* Save badge */}
              <div className="absolute bottom-4 left-4">
                <span className="bg-red-500 text-white font-black text-sm px-4 py-1.5 rounded-md">SAVE {l.savingsPct}%</span>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              <div className="flex items-start justify-between gap-3 mb-4">
                <div>
                  ...
                  <h1 className="text-xl font-bold text-gray-900">{l.address}</h1>
              ...
              <div className="bg-slate-50 rounded-xl p-4 mb-4">
                <div className="flex items-end justify-between mb-2">
                  <div>
                    ...
                    <p className="text-3xl font-black text-[#1e3a5f]">{formatRM(l.reservePrice)}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-xs text-gray-400 mb-0.5"><T en="Market Value" bm="Nilai Pasaran" /></p>
                    <p className="text-lg font-bold text-gray-400 line-through">{formatRM(l.marketValue)}</p>
                  </div>
                </div>
                {/* RP vs MV bar */}
                ...
                <div className="flex justify-between text-[11px] mt-1">
                  ...
                  <span className="text-red-500 font-semibold">Save {formatRM(l.savings)} ({l.savingsPct}% off)</span>
                </div>
              </div>
            ...
            <div className="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100">
              <div className="px-5 py-4 border-b border-gray-100">
                ...
                <p className="text-gray-500 text-xs mt-0.5">{l.address}</p>
              </div>
              <div className="h-56">
                <iframe src={mapSrc} width="100%" height="100%" className="border-0" allowFullScreen loading="lazy" referrerPolicy="no-referrer-when-downgrade" title={`Map of ${l.address}`} />
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 sticky top-20">
              <p className="font-bold text-gray-900 mb-1"><T en="Interested in this property?" bm="Berminat dengan hartanah ini?" /></p>
              <p className="text-xs text-gray-500 mb-4"><T en="Contact our team for full due diligence and auction assistance." bm="Hubungi pasukan kami untuk usaha wajar penuh dan bantuan lelongan." /></p>
        ...
        {similar.length > 0 && (
          <div className="mt-12 border-t border-gray-100 pt-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
              {similar.map((s) => (
                <Link key={s.id} href={`/auction/${s.id}`} className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all overflow-hidden flex flex-col">
