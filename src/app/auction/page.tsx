...
export default async function AuctionPage({
  ...
  searchParams: Promise<{ q?: string; region?: string; category?: string; sort?: string }>;
}) {
  const { q, region, category, sort } = await searchParams;
  ...
  const hasFilters = q || (region && region !== 'All') || (category && category !== 'All');
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="bg-white border-b border-gray-100 shadow-sm sticky top-16 z-40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3">
          <form method="GET" action="/auction" className="flex flex-wrap gap-2 items-center">
            {/* Search */}
            <input
              name="q"
              defaultValue={q || ''}
              placeholder="Address, area, property type…"
              className={`${inp} flex-1 min-w-0 sm:w-64 sm:flex-none`}
            />
            {/* Region */}
            ...
            <button
              type="submit"
              className="bg-amber-500 hover:bg-amber-600 text-white font-bold text-sm px-5 h-10 rounded-xl transition-colors shrink-0"
            >
              <T en="Search" bm="Cari" />
            </button>
            {/* Clear */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        {filtered.length === 0 ? (
          <div className="text-center py-20 text-gray-400">
            <p className="font-semibold text-gray-500 mb-2"><T en="No listings match your filters" bm="Tiada senarai sepadan dengan tapisan anda" /></p>
